function [ t, x ] = directMethod_probaribo( stoich_matrix, propensity_fcn, tspan, x0,...
...
% When a ribo passes through, the degradation pathway can be activated with a fixed
% probability p_delta
%   Gregory Roth, 2022
...
%%
    ...
    while T(reac_count) < tspan(2)        
        ...
        if mu==ini_reaction %check if a ribosome has been loaded
            r_delta=rand(1);
            if r_delta<=prob_delta %nmd is initiated with proba prob_delta
