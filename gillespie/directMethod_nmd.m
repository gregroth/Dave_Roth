function [ t, x ] = directMethod_nmd( stoich_matrix, propensity_fcn, tspan, x0,...
                                  params,ini_reaction,prob_nmd}
%% ATTENTION this is a modified version of the directMethod function specially written for the nmD degradation pathway
% When a ribo passes through, the nmD pathway can be activated with a fixed
% probability p_nmd
...
%%
    ...
    while T(reac_count) < tspan(2)        
        ...
        if mu==ini_reaction %check if a ribosome has been loaded
            r_nmd=rand(1);
            if r_nmd<=prob_nmd %nmd is initiated with proba prob_nmd
