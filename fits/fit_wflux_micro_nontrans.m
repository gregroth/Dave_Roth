%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Fit the micro rna model with translation independent rate
%
%
% Inputs:
%
...
sigma=1;
N=length(timepts);
MLF_tot=@(x)N*log(sigma*sqrt(2*pi))+sum((datatofit-rna_traj_micro_noHistory([bestfit_parameter,x(1),x(1)],nb_rna,nb_nuc,timepts)).^2)/(2*sigma^2);     
%parameter limits
lb=0.3;%     x =[deltaon,deltaoff]
ub=2;
...
bestfit_parameter=[bestfit_parameter,x(1),x(1)];
%save
filename=strcat('./bestfits/bestfit_let7_nontrans_noHistory_',expnamelist{1},'_',erase(num2str(cl([1,2,3,4,5]))," "),'_',conditionlist{1});
